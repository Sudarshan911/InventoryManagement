...
app.put("/cancelorder", (request, response) => {
    ...
    database.collection("PlacedOrders").find({ "customer_name": request.body.customer_name, "product": request.body.product }).toArray()
        .then(
            (res) => {
                ...
                if (res[0].status == "confirmed") {
                    ...
                    return database.collection("PlacedOrders").update({ "customer_name": request.body.customer_name, "product": request.body.product }, { $set: { "status": "cancelled" } })
                        //here
                        .then(
                            (res) => {
                                // Using the "Placedordered collection to identify the respective Inventory document to be updated"
                                return database.collection("Placedorderwarehouse").find({ "name": request.body.customer_name }).toArray()
                        ...
                        .then(
                            (res) => {
                                return database.collection("Inventory").update({ "product": request.body.product, "Warehouse": res[0].warehouse }, {
                                    //updating the quantity in Inventory
                                    $inc: { "InStock": request.body.quantity_requested }
